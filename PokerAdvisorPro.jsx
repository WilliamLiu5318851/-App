...
const getStraightFlushHigh = (cards) => {
  ...
  let ranks = suits[flushSuit].sort((a, b) => b - a); 
      ...
      <div className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-30 shadow-md">
        <div className="max-w-xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2 text-xs font-medium items-center">
            <button onClick={cycleStrategy} className={`px-3 py-1.5 rounded-full transition border flex items-center gap-1 ${getStrategyStyle()}`}>
              {strategy === 'maniac' && <Flame className="w-3 h-3 animate-pulse" />}
              {getStrategyLabel()}
            </button>
            <button onClick={() => setShowSettings(true)} className="bg-slate-800 p-2 rounded-full hover:bg-slate-700 border border-slate-700 text-slate-400 hover:text-white transition"><Settings className="w-4 h-4" /></button>
            <button onClick={() => setLang(l => l === 'zh' ? 'en' : 'zh')} className="bg-slate-800 px-3 py-1.5 rounded-full hover:bg-slate-700 border border-slate-700">{lang === 'zh' ? 'EN' : '中'}</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 shadow-inner grid grid-cols-2 gap-4">
          ...
          <div className="text-right">
            <div className="text-slate-500 text-xs mb-1 uppercase tracking-wider flex items-center justify-end gap-1">{t.spr} <Info className="w-3 h-3"/></div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center px-1">
             ...
             {street < 3 ? (
               <button onClick={nextStreet} className="text-xs bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1 rounded-full flex items-center gap-1 transition shadow-lg shadow-emerald-900/50">{t.nextStreet} <ArrowRight className="w-3 h-3" /></button>
             ) : (
               !settlementMode && (<button onClick={enterSettlement} className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1 rounded-full flex items-center gap-1 transition">{t.finishHand}</button>)
             )}
          </div>
          <div className="flex gap-2 h-20 sm:h-24">
             {[0,1,2].map(i => (
               <div key={i} onClick={() => street >= 1 && handleCardClick('board', i)} className={`flex-1 rounded-lg border-2 flex items-center justify-center cursor-pointer transition relative ${street >= 1 ? 'bg-slate-800 border-slate-600 hover:border-slate-400' : 'bg-slate-900/50 border-slate-800 opacity-30 cursor-not-allowed'}`}>
             ...
             {[3,4].map(i => (
                <div key={i} onClick={() => street >= (i===3?2:3) && handleCardClick('board', i)} className={`flex-1 rounded-lg border-2 flex items-center justify-center cursor-pointer transition relative ${street >= (i===3?2:3) ? 'bg-slate-800 border-slate-600 hover:border-slate-400' : 'bg-slate-900/50 border-slate-800 opacity-30 cursor-not-allowed'}`}>
                   {communityCards[i] ? <CardIcon rank={communityCards[i].rank} suit={communityCards[i].suit} className="w-full h-full" /> : <span className="text-slate-700 text-xs">{i===3?'Turn':'River'}</span>}
                </div>
             ))}
          </div>
        </div>
              ...
              <div className="flex-1 flex flex-col justify-center space-y-3">
                 <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      {heroStack === 0 && (<button onClick={() => setHeroStack(buyInAmount)} className="flex items-center gap-1 text-[10px] bg-emerald-600 hover:bg-emerald-500 text-white px-2 py-0.5 rounded shadow animate-pulse"><RotateCcw className="w-3 h-3" /> {t.rebuy} {buyInAmount}</button>)}
                    </div>
                    <input type="number" value={heroStack === 0 ? '' : heroStack} onChange={e => handleStackChange(e.target.value)} placeholder="0" className={`w-full bg-slate-950 border rounded px-2 py-1 font-mono transition focus:outline-none ${heroStack === 0 ? 'border-red-500 text-red-400 placeholder-red-700' : 'border-slate-700 text-yellow-400'}`} />
                 </div>
                 <div>
                    <div className="flex justify-between items-center mb-1">
               ...
               <div key={p.id} className={`flex items-center gap-3 bg-slate-800 p-2 rounded-lg border ${p.active ? 'border-slate-700' : 'opacity-50 border-transparent'}`}>
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-2">
                     ...
                     <div className="flex items-center gap-2 bg-slate-900 rounded px-2">
                        ...
                        <input type="number" value={p.bet === 0 ? '' : p.bet} placeholder="0" onChange={e => handleOpponentBetChange(p.id, e.target.value)} className="w-full bg-transparent text-white text-sm py-1 font-mono focus:outline-none" />
                     </div>
                  </div>
...
<<<<<<< HEAD
                   ...
                   <div className="flex flex-wrap gap-2 my-1">
                     {result.handTypeLabel && (
                       <div className="inline-block bg-slate-700 text-blue-200 text-xs px-2 py-0.5 rounded border border-blue-500/30">
                         <span className="flex items-center gap-1"><Lightbulb className="w-3 h-3"/> {result.handTypeLabel}</span>
                       </div>
                     )}
                     {result.textureLabel && (
                       <div className="inline-block bg-slate-700 text-indigo-200 text-xs px-2 py-0.5 rounded border border-indigo-500/30">
                         <span className="flex items-center gap-1"><Grid className="w-3 h-3"/> {result.textureLabel}</span>
                       </div>
                     )}
                   </div>
...
>>>>>>> parent of 83c450a (Update PokerAdvisorPro.jsx)
             ...
             {result.betSizes && (
               <div>
                 <div className="px-4 pt-2 pb-1 text-xs text-slate-400 flex items-center gap-1"><MousePointerClick className="w-3 h-3" /> {t.betSizing}</div>
                 <div className="p-4 grid grid-cols-3 gap-3 bg-slate-800/30">
                    {Object.entries(result.betSizes).map(([size, amount]) => (
                        <button key={size} onClick={() => setHeroBet(amount)} className={`flex flex-col items-center p-2 rounded hover:bg-slate-700/50 active:bg-slate-700 transition border border-transparent hover:border-slate-600 ${size === 'med' ? 'border-x border-slate-700/50' : ''}`}>
                          <div className="text-xs text-slate-500 mb-1">{t[`bet_size_${size === 'small' ? 'small' : size === 'med' ? 'med' : strategy === 'maniac' ? 'over' : 'large'}`]}</div>
                          <div className={`font-mono font-bold ${strategy === 'maniac' && size === 'large' ? 'text-purple-300' : amount === heroStack ? 'text-red-400' : 'text-blue-300'}`}>{amount} {amount === heroStack && <span className="text-[10px] block">(All-In)</span>}</div>
                        </button>
                    ))}
                 </div>
               </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowSettings(false)}>
           <div className="bg-slate-800 rounded-xl p-6 max-w-sm w-full border border-slate-600 shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
              ...
              <div>
                 ...
                 <input type="range" min="1" max="8" step="1" value={deckCount} onChange={(e) => setDeckCount(Number(e.target.value))} className="w-full accent-blue-500" />
                 <div className="flex justify-between text-xs text-slate-500 mt-1 font-mono"><span>1</span><span>8</span></div>
                 <p className="text-xs text-slate-500 mt-2 bg-slate-900/50 p-2 rounded border border-slate-700/50 flex gap-2"><Info className="w-4 h-4 shrink-0 mt-0.5" /> {t.deck_info}</p>
              </div>
              <div>
                 ...
                 <div className="flex items-center bg-slate-900 rounded border border-slate-700">
                    ...
                    <input type="number" value={buyInAmount === 0 ? '' : buyInAmount} onChange={(e) => handleBuyInChange(e.target.value)} placeholder="0" className="w-full bg-transparent py-2 text-white font-mono focus:outline-none" />
                 </div>
                 <p className="text-xs text-slate-500 mt-2 bg-slate-900/50 p-2 rounded border border-slate-700/50 flex gap-2"><Info className="w-4 h-4 shrink-0 mt-0.5" /> {t.buy_in_info}</p>
              </div>
           </div>
        </div>
      )}
